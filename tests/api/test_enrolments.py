...
from app.domain.entities.enrolment_request import EnrolmentRequest
...
@mock.patch("app.use_cases.get_enrolments.GetEnrolments")
def test_get_enrolments_success(use_case):
    message = "The enrolment requests have been processed."
    ...
    enrolment = EnrolmentRequest(**enrolments[0])
    enrolment1 = EnrolmentRequest(**enrolments[1])
...
def test_process_enrolments_success(use_case):
    ...
    json_result = response.json()
    enrolment = EnrolmentRequest(**json_result.get("value"))
